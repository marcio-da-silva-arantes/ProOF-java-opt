...
//        this.value = bound.valid(value+extra);
//    }
//    
    @Override
    ...
    @Override
    public double deviation(Obj other) {
        if(this.compareTo(other)!=0){
            return (this.abs_value() - other.abs_value())/(Math.abs(other.abs_value()) + 1e-6);
        }
        return 0;
    }
    @Override
    public double gap(Obj other) {
        if(this.compareTo(other)!=0){
            return (this.abs_value() - other.abs_value())/(Math.abs(this.abs_value()) + 1e-6);
        }
        return 0;
    }
